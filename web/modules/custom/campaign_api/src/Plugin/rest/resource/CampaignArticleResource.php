...
/**
 * Provides a resource for a campaign article.
 * @RestResource(
 *   id = "campaignarticle",
 *   label = @Translation("Campaign Article Collection"),
 *   uri_paths = {
 *     "canonical" = "/v0/campaign/{id}/article/{aid}"
 *   }
 * )
 *
 */
class CampaignArticleResource extends ResourceBase {
  ...
  public function get($id, $aid, $unserialized = NULL, Request $request) {
    if ($id && is_numeric($id)) {
      $build = [];
      ...
      if ($nid != $aid) {
        throw new NotFoundHttpException(t('No article ID supplied.'));
      }
    ...
    throw new NotFoundHttpException(t('No campaign ID supplied.'));
  }
